...
function replaceExact(pattern, replacement) {
  if (content.includes(pattern)) {
    ...
    console.log("Success replacing exact block.");
  } else {
    console.error("FAILED to find exact block:\n", pattern.substring(0, 50) + "...");
    process.exit(1);
  }
}
...
if (saveRegex.test(content)) {
  ...
  console.log("Success replacing handleSavePrompts.");
} else {
  console.error("FAILED to find handleSavePrompts.");
  process.exit(1);
}
...
if (addRegex.test(content)) {
  ...
  console.log("Success replacing handleAddSubject.");
} else {
  console.error("FAILED to find handleAddSubject.");
  process.exit(1);
}
...
console.log("Successfully rewrote TeacherDashboard.jsx start to finish!");
