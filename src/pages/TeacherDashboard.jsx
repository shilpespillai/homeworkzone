                ...
                <div className="px-6 py-6 space-y-6 pb-20 relative min-h-[calc(100vh-64px)] bg-[#FAF9FF]">
                   ...
                   <div className="grid grid-cols-12 gap-6">
                      ...
                      <div className="col-span-4 space-y-6">
                         ...
                         {/* Student Homework Track Status */}
                         <div className="bg-gradient-to-br from-[#FFF5F8] to-[#FFE8ED] rounded-[32px] border border-[#FFDCE5] shadow-sm p-6 space-y-4">
                            <div className="flex items-center gap-3">
                               <span className="text-3xl">🎯</span>
                               <div className="space-y-0.5">
                                  <h3 className="text-xl font-black text-[#8E1F41] tracking-tight">Homework Tracking</h3>
                                  <p className="text-[9px] font-black text-[#D83A6F] uppercase tracking-widest">Students on track vs lagging</p>
                               </div>
                            </div>
                            ...
                            <div className="space-y-4">
                               ...
                               <div className="space-y-2">
                                  <div className="flex justify-between items-end">
                                     <span className="text-xs font-black text-[#8E1F41]">{onTrackPercent}% On Track</span>
                                     <span className="text-[10px] font-bold text-[#D83A6F]">{onTrackCount} of {totalDashboardStudents} students</span>
                                  </div>
                                  <div className="h-4 w-full bg-white rounded-full overflow-hidden border border-[#FFDCE5] flex">
                                     <div className="h-full bg-[#FF477E] transition-all duration-1000" style={{ width: `${onTrackPercent}%` }} />
                                     <div className="h-full bg-[#FFB3C6] transition-all duration-1000" style={{ width: `${100 - onTrackPercent}%` }} />
                                  </div>
                               </div>

                               {/* Lagging Summary */}
                               {laggingCount > 0 ? (
                                  <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 border border-[#FFDCE5] shadow-sm space-y-3">
                                     <div className="flex items-center justify-between">
                                        <span className="text-xs font-black text-[#8E1F41] flex items-center gap-1">
                                           ⚠️ <span className="text-[#D83A6F]">{laggingCount}</span> students lagging
                                        </span>
                                        <button 
                                           onClick={() => setDashboardRosterTab('Missing')} 
                                           className="text-[9px] font-black uppercase text-[#FF477E] hover:text-[#D83A6F] bg-[#FFF5F8] px-2 py-1 rounded-lg transition-all"
                                        >
                                           View All
                                        </button>
                                     </div>
                                     <div className="flex flex-wrap gap-2">
                                        {dashboardMissingReports.slice(0, 5).map(report => (
                                           <div key={report.student.name} className="flex items-center gap-1.5 bg-[#FFF5F8] px-2.5 py-1 rounded-xl border border-[#FFDCE5]">
                                              <img src={getStudentAvatar(report.student.name)} className="w-5 h-5 rounded-full border border-white" />
                                              <span className="text-[10px] font-black text-[#8E1F41]">{report.student.name}</span>
                                              <span className="text-[9px] font-bold text-[#D83A6F] bg-white px-1.5 rounded-md">
                                                 {report.missingHws.length}
                                              </span>
                                           </div>
                                        ))}
                                        {laggingCount > 5 && (
                                           <div className="flex items-center px-2 py-1 rounded-xl bg-white border border-[#FFDCE5]">
                                              <span className="text-[9px] font-black text-[#8E1F41]">+{laggingCount - 5} more</span>
                                           </div>
                                        )}
                                     </div>
                                  </div>
                               ) : (
                                  <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 border border-[#FFDCE5] shadow-sm text-center space-y-1">
                                     <span className="text-2xl block">🌟</span>
                                     <p className="text-xs font-black text-[#8E1F41]">Everyone is caught up!</p>
                                     <p className="text-[10px] font-bold text-[#D83A6F]">No missing homework across the board.</p>
                                  </div>
                               )}
                            </div>
                         </div>
                         ...
                         <div className="bg-white rounded-[32px] border border-[#E9E4FF] shadow-sm p-6 space-y-4">
                            <div className="flex justify-between items-center border-b border-[#FAF2FF] pb-2">
                               ...
                               <div className="flex bg-[#F5F3FF] p-1 rounded-xl border border-[#FFEDD5]">
                                  <button 
                                  ...
                                  <button 
                                     onClick={() => setDashboardRosterTab('Missing')} 
                                     className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase transition-all ${dashboardRosterTab === 'Missing' ? 'bg-white text-rose-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                  >
                                     ⏳ Missing HW ({dashboardMissingReports.length})
                                  </button>
                               </div>
                            </div>
                            
                            <div className="space-y-3 max-h-[160px] overflow-y-auto no-scrollbar">
                               {dashboardRosterTab === 'Support' && struggling.map(st => (
                               ...
                               {dashboardRosterTab === 'Flyers' && risingStars.map(st => (
                                  <div key={st.name} className="flex items-center justify-between border-b border-[#FAF2FF] pb-2">
                                     <div className="flex items-center gap-2">
                                        <img src={getStudentAvatar(st.name)} className="w-8 h-8 rounded-full border border-slate-100 bg-white" />
                                        <span className="text-xs font-black text-[#5C4D9F]">{st.name}</span>
                               ...
                               {dashboardRosterTab === 'Missing' && dashboardMissingReports.map(report => (
                                  <div key={report.student.name} className="flex flex-col border-b border-[#FAF2FF] pb-2 space-y-2">
                                     <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                           <img src={getStudentAvatar(report.student.name)} className="w-8 h-8 rounded-full border border-slate-100 bg-white" />
                                           <span className="text-xs font-black text-[#5C4D9F]">{report.student.name}</span>
                                        </div>
                                        <span className="text-[10px] font-black text-rose-500 bg-rose-50 px-2 py-0.5 rounded-full">{report.missingHws.length} missing</span>
                                     </div>
                                     <div className="flex flex-wrap gap-1 pl-10">
                                        {report.missingHws.map(hw => (
                                           <span key={hw.id} className="text-[8px] font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded uppercase tracking-wider truncate max-w-[120px]" title={hw.title}>
                                              {hw.title}
                                           </span>
                                        ))}
                                     </div>
                               ...
                               {dashboardRosterTab === 'Missing' && dashboardMissingReports.length === 0 && (
                                  <div className="text-xs text-emerald-500 font-black italic text-center py-4">All homework submitted! 🎉</div>
                               )}
                            </div>
                         </div>
                      </div>
