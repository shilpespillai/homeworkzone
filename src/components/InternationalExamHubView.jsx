...
export default function InternationalExamHubView({ onSelectExam, onBack }) {
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 space-y-6 font-sans animate-fadeIn">
      ...
      {/* Top Banner Header */}
      <div className="bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 rounded-[40px] p-8 md:p-12 text-white shadow-xl border-4 border-emerald-300/40 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4 max-w-3xl">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/20 border border-white/30 text-amber-300 text-xs font-black uppercase tracking-widest backdrop-blur-md">
            <Trophy className="w-4 h-4 text-amber-300" /> Authentic Examination Blueprints
          </div>
        ...
        <div className="flex flex-wrap gap-3 pt-6 relative z-10">
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl font-extrabold text-xs transition-all active:scale-95 ${countryFilter === tab.id ? 'bg-amber-400 text-slate-900 shadow-md font-black' : 'bg-white/15 hover:bg-white/25 text-white border border-white/20'}`}
            >
              {tab.label}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {filteredExams.map((exam) => (
          <div
            key={exam.id}
            className="bg-white rounded-2xl border-2 border-slate-200/80 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col justify-between overflow-hidden group hover:-translate-y-0.5 hover:border-emerald-400 p-4"
          >
            {/* Card Top Content */}
