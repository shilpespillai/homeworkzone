...
export default function FlagQuizView({ onAddPoints }) {
  ...
  const generateQuestions = () => {
    ...
    const newQuestions = shuffledPool.map((targetCountry) => {
      ...
      const rawOptions = [targetCountry, ...wrongOptions].sort(() => Math.random() - 0.5);

      // Format options depending on question type
  ...
  // Start new game on continent/mode/quizCategory change
  useEffect(() => {
    if (mode !== 'study') {
      generateQuestions();
    }
  }, [selectedContinent, mode, quizCategory]);
      ...
      {mode === 'study' ? (
        ...
        <div className="bg-white border border-slate-200 rounded-[36px] p-8 md:p-12 text-center shadow-xl max-w-2xl mx-auto space-y-6 animate-in zoom-in-95 duration-300">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            ...
            >
              <BookOpen className="w-4 h-4" /> Study Atlas First
            </button>
          </div>
        </div>
      ) : (
        ...
        questions.length > 0 && currentQ && (
          <div className="space-y-6">
            ...
            <div className="bg-white border-2 border-slate-200/80 rounded-[36px] p-6 md:p-10 shadow-xl space-y-8 text-center relative overflow-hidden">
              ...
              <div className="space-y-2">
                <span className="text-xs md:text-sm font-black text-blue-700 uppercase tracking-widest bg-blue-50 px-4 py-1.5 rounded-full border border-blue-100 inline-block shadow-sm">
              ...
              {currentQ.type === 'countryFromCapital' ? (
                <div className="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-200/80 rounded-3xl p-6 max-w-md mx-auto shadow-inner space-y-2">
                  ...
                  <p className="text-xs font-bold text-amber-700">Capital City</p>
                </div>
              ) : (
                  ...
                  return (
                    ...
                    >
                      <span className="flex items-center gap-3">
                        <span className="w-8 h-8 rounded-full bg-black/5 flex items-center justify-center text-xs font-bold text-slate-500 shrink-0">
                          {option.flag || '🏛️'}
                        </span>
                        <div>
                          <div>{option.displayLabel}</div>
                          {option.subLabel && (
                            <div className="text-xs font-medium opacity-80">{option.subLabel}</div>
                          )}
                        </div>
                      </span>
