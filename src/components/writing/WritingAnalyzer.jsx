              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black flex items-center gap-2">
                    <span>Saved Essays & Exemplar Reports</span>
                    {isSyncing && <Loader2 className="w-4 h-4 animate-spin text-indigo-400" />}
                  </h3>
                  <p className="text-slate-400 text-xs font-medium">
                    All previously analyzed pieces are preserved in the cloud. Click any piece to reopen.
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  onClick={fetchCloudSavedEssays}
                  disabled={isSyncing}
                  className="px-3 py-1.5 rounded-xl bg-white/10 hover:bg-white/20 text-white font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50"
                  title="Sync with cloud database"
                >
                  <RefreshCcw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin text-yellow-400' : ''}`} />
              ...
              {filteredHistory.length === 0 ? (
                <div className="text-center py-12 space-y-4">
                  ...
                  <p className="text-xs text-slate-500 max-w-sm mx-auto">
                    When you analyze writing drafts, they will automatically be preserved in the cloud so you can revisit them anytime!
                  </p>
                  <button
                    onClick={fetchCloudSavedEssays}
                    disabled={isSyncing}
                    className="px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs inline-flex items-center gap-2 cursor-pointer shadow-md disabled:opacity-50"
                  >
                    <RefreshCcw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
                    <span>{isSyncing ? 'Checking Cloud...' : 'Refresh from Cloud'}</span>
                  </button>
                </div>
              ) : (
