...
// 4. handleSavePrompts
const saveTarget = `  const handleSavePrompts = async () => {
    if (!user?.uid) return;
    setIsSavingPrompts(true);
    try {
      await updateDoc(doc(db, 'teachers', user.uid), {
...
// For this one, the emojis might be problematic due to utf-8. Let's use Regex.
if(content.includes("Generic Subject Prompts saved successfully!")) {
  content = content.replace(/  const handleSavePrompts = async \(\) => \{[\s\S]*?alert\("Failed to save prompts\. ❌"\);\n    \}\n    setIsSavingPrompts\(false\);\n  \};/, saveNew);
  console.log("Success replacing handleSavePrompts via Regex.");
}

// 5. handleAddSubject
const addTarget = `  const handleAddSubject = async () => {
...
content = content.replace(/  const handleAddSubject = async \(\) => \{[\s\S]*?alert\("AI failed to generate a specialized prompt\. We saved a default premium template instead! 🔧"\);\n    \}\n  \};/, addNew);
...
console.log("Successfully rewrote TeacherDashboard.jsx with all Option 3 features.");
