...
// 1. Inject Loader2 import
file = file.replace(/  CreditCard,\r?\n/, '  CreditCard,\n  Loader2,\n');

// 2. Inject the loading overlay
const searchStr =           case 'Tuition Fees': {\r?\n             return \\(\r?\n                <div className="px-10 py-10 space-y-8 min-h-\\[calc\\(100vh-64px\\)\\] pb-40">;

const replaceStr =           case 'Tuition Fees': {
             return (
                <div className="px-10 py-10 space-y-8 min-h-[calc(100vh-64px)] pb-40">
                  {/* Stripe Loading Overlay */}
                  {isRedirectingStripe && (
                    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
                      <div className="bg-white rounded-[40px] p-10 max-w-sm w-full shadow-2xl flex flex-col items-center text-center space-y-6">
...
file = file.replace(new RegExp(searchStr), replaceStr);

fs.writeFileSync('src/pages/TeacherDashboard.jsx', file, 'utf-8');
console.log('Spinner overlay injected!');
